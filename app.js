...
/* -------------------------------------------------------------------------
   [CSS 교정본: 사람이름 미색 고정 + 세로 하이라이트 쨍하게]
   ------------------------------------------------------------------------- */
...
html, body {
  margin: 0; width: 100%; height: 100%; overflow: hidden; overflow-x: hidden;
  font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Pretendard", "Segoe UI", sans-serif;
  background: #e4e7eb; color: #111827;
  -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;
}
body { overscroll-behavior: none; transition: background-color 0.2s ease, color 0.2s ease; }
button, input, select { font: inherit; }
button { transition: transform 0.14s ease, opacity 0.18s ease, background-color 0.18s ease, box-shadow 0.18s ease, border-color 0.18s ease; }
button:active { transform: scale(0.97); }
#root { width: 100%; height: 100%; overflow-x: hidden; }

.container { width: 100%; min-height: 100dvh; padding: 24px 8px 24px; overflow-y: auto; overflow-x: hidden; touch-action: pan-y; }
.container::after { content: ""; display: block; height: calc(86px + env(safe-area-inset-bottom, 0px)); flex-shrink: 0; }

.card { background: rgba(250, 250, 248, 0.9); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border-radius: 24px; padding: 14px; margin-bottom: 10px; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.65); border: 1px solid rgba(0, 0, 0, 0.05); }
.card-title { font-size: 20px; font-weight: 800; margin-bottom: 10px; }
.help-text { color: #6b7280; font-size: 14px; line-height: 1.5; margin-top: 6px; }

.notice-box { background: rgba(233, 242, 255, 0.95); border: 1px solid rgba(189, 210, 255, 0.95); color: #2146a6; border-radius: 16px; padding: 10px 12px; margin-top: 10px; font-size: 13px; }
.label { display: block; margin-bottom: 6px; color: #4b5563; font-size: 14px; font-weight: 700; }
.input, .select { width: 100%; padding: 11px 12px; border-radius: 16px; border: 1px solid rgba(203, 213, 225, 0.95); background: rgba(255, 255, 255, 0.95); outline: none; }

.settings-row { min-height: 52px; display: flex; justify-content: flex-end; align-items: center; gap: 8px; margin-bottom: 8px; flex-wrap: wrap; }
.settings-btn, .install-btn, .quick-btn { border: 0; color: white; height: 42px; padding: 0 14px; border-radius: 16px; cursor: pointer; font-weight: 800; font-size: 14px; display: inline-flex; align-items: center; justify-content: center; gap: 7px; white-space: nowrap; }
.settings-btn { background: linear-gradient(180deg, #4b7dff 0%, #2f61dc 100%); }
.install-btn { background: linear-gradient(180deg, #2aa55d 0%, #197a42 100%); }
.quick-btn.band { background: linear-gradient(180deg, #21b35a 0%, #169447 100%); }
.quick-btn.vacation { background: linear-gradient(180deg, #2ccf63 0%, #1db954 100%); }
.quick-icon { width: 20px; height: 20px; object-fit: contain; }

.date-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 10px; height: 168px; }
.date-box { background: rgba(255, 255, 255, 0.78); display: grid; grid-template-rows: 46px 1fr 46px; overflow: hidden; border-radius: 22px; box-shadow: 0 8px 20px rgba(15, 23, 42, 0.08), border: 1px solid rgba(255, 255, 255, 0.8); }
.date-btn { width: 100%; height: 46px; border: 0; background: rgba(215, 220, 230, 0.55); font-size: 26px; font-weight: 800; cursor: pointer; }
.date-value { background: rgba(255, 255, 255, 0.95); display: flex; align-items: center; justify-content: center; text-align: center; font-size: 23px; padding: 4px; }

.main-panel { min-height: 210px; background-color: rgba(250, 250, 248, 0.92); border-radius: 28px; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255, 255, 255, 0.8); }
.main-code { font-size: clamp(40px, 9vw, 56px); font-weight: 800; letter-spacing: -0.02em; }
.main-time { margin-top: 14px; font-size: clamp(24px, 6vw, 36px); font-weight: 700; }
.main-subinfo { margin-top: 12px; font-size: 15px; font-weight: 700; color: #64748b; }

.home-path-preview { margin-top: 18px; display: flex; flex-direction: column; align-items: center; cursor: pointer; }
.home-path-preview img { width: 100%; max-width: 240px; height: 130px; object-fit: contain; border-radius: 14px; background: #f8fafc; border: 1px solid rgba(203, 213, 225, 0.8); box-shadow: 0 6px 16px rgba(15, 23, 42, 0.08); }
.preview-label { margin-top: 8px; font-size: 11px; color: #475569; font-weight: 700; background: rgba(241, 245, 249, 0.8); padding: 4px 12px; border-radius: 12px; }

.bottom-tabs { position: fixed; left: 0; right: 0; bottom: 0; height: calc(60px + env(safe-area-inset-bottom, 0px)); background: rgba(39, 82, 191, 0.92); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); z-index: 50; display: flex; }
.bottom-tabs.home-theme { background: rgba(39, 82, 191, 0.92); }
.bottom-tabs.all-theme { background: rgba(59, 130, 246, 0.92); }
.bottom-tabs.month-theme { background: rgba(16, 185, 129, 0.92); }
.bottom-tabs.group-theme { background: rgba(139, 92, 246, 0.92); }
.bottom-tab { flex: 1; border: 0; background: transparent; color: rgba(255, 255, 255, 0.84); font-size: 15px; font-weight: 600; cursor: pointer; }
.bottom-tab.active { color: #ffffff; background: rgba(255, 255, 255, 0.08); font-weight: 700; }

/* 🚀 [교정] 사람이름 미색 고정 + 세로줄 하이라이트 CSS */
.group-table-wrap { overflow: hidden; border-radius: 18px; background: #ffffff; border: 1px solid #c8d2e3 !important; }
.group-table { width: 100%; border-collapse: separate; border-spacing: 0; font-size: 14px; }
.group-table th, .group-table td { border: 1px solid #c8d2e3 !important; }

/* 이름 칸 미색 고정 (하이라이트 방지) */
.group-table th.sticky-col, .group-table td.sticky-col, .group-name-cell { 
  position: sticky; left: 0; 
  background-color: #ffffff !important; /* 👈 라이트모드 미색(흰색) 고정 */
  z-index: 10; border-right: 2px solid #e2e8f0 !important; padding: 8px 4px !important; width: 76px; min-width: 76px;
}
.dark-mode .group-table th.sticky-col, .dark-mode .group-table td.sticky-col { 
  background-color: #1e293b !important; /* 👈 다크모드 네이비 고정 */
}

/* 세로 하이라이트 쨍하게 (사진 연한 하늘색) */
.group-table th.active-col, .group-table td.active-col { 
  background-color: #f0f9ff !important; 
}
.dark-mode .group-table th.active-col, .dark-mode .group-table td.active-col { 
  background-color: rgba(0, 242, 255, 0.15) !important; 
}

/* 오늘 날짜 (세로 하이라이트보다 우선순위) */
.group-table th.today-col { border-bottom: 4px solid #3b82f6 !important; background-color: #fff7ed !important; }

.team-badge { background: transparent !important; color: #64748b; font-size: 11px; font-weight: 600; }
.name-txt { font-weight: 800; font-size: 14px; color: #111827; }
.dark-mode .name-txt { color: #f8fafc; }

/* 월교번 디자인 */
.month-calendar { background: rgba(255, 255, 255, 0.76); border-radius: 24px; padding: 10px; border: 1px solid #c8d2e3 !important; }
.month-cell { border: 1px solid #c8d2e3 !important; background: #ffffff; min-height: 116px; padding: 0; }
.month-cell.selected { border: 3px solid #00f2ff !important; z-index: 10; position: relative; }
.month-day { font-size: 15px; font-weight: 800; }
.month-code-line { font-size: 17.5px; font-weight: 900; }

.modal-backdrop { position: fixed; inset: 0; background: rgba(17, 24, 39, 0.34); backdrop-filter: blur(10px); display: flex; align-items: center; justify-content: center; z-index: 4000; padding: 16px; }
.modal { width: 100%; max-width: 520px; background: rgba(255, 255, 255, 0.95); border-radius: 24px; padding: 20px; }

.dark-mode { background: #0f172a; color: #f8fafc; }
.dark-mode .card, .dark-mode .main-panel, .dark-mode .modal { background: rgba(30, 41, 59, 0.95); border-color: #334155 !important; }
`;
