 ...
 * 2. 검색 필터 유지: 2000대 열차 검색 시 searchOrigin을 통해 이미지 뷰어까지 어제 날짜 정보 전달 (searchPathDate 도입)
 * 3. 변수 선언 보정: getMonthMatrix 내 year 변수 선언 누락 재확인 및 수정
 * 4. 원본의 모든 기능, 레이아웃, 관리자 및 그룹 로직 100% 그대로 유지 (압축 없음)
 **/
...
function saveHolidayYearToCache(year, dates) {
  try {
    localStorage.setItem(`${LS_HOLIDAY_CACHE_PREFIX}${year}`, JSON.stringify({
      year,
      savedAt: Date.now(),
...
function parseInfo(text) {
  ...
  return {
    raw: lines,
    baseDate: year && month && day ? `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}` : null,
...
function parseWorktime(text, gyobunOrder = []) {
  ...
  gyobunOrder.forEach((code, idx) => {
    map[String(code || "").trim().toLowerCase()] = lines[idx] || "----";
  });
  return map;
}
...
function getNightRange(teamKey) {
  return NIGHT_RANGE_BY_TEAM[teamKey] || {
    start: 22,
    end: 29
  };
}
...
function getWorktimeOverrideKey(teamKey, personName) {
...
function buildTimeValueFromParts(sh, sm, eh, em) {
  ...
  if (Number.isNaN(shNum) || Number.isNaN(smNum) || Number.isNaN(ehNum) || Number.isNaN(emNum) || shNum < 0 || shNum > 23 || ehNum < 0 || ehNum > 23 || smNum < 0 || smNum > 59 || emNum < 0 || emNum > 59) return null;
...
function getGyobunOrder(team) {
  return team?.gyobun?.length ? team.gyobun : DEFAULT_GYOBUN;
}

function getDiaOrder(team) {
  return team?.diaOrder?.length ? team.diaOrder : getGyobunOrder(team);
}

function normalizeToFixedCode(team, code) {
  const fixedCodes = getGyobunOrder(team);
  return fixedCodes.find((item) => normalizeCodeKey(item) === normalizeCodeKey(code)) || code || "";
}

function shiftCodeByDays(team, baseCode, dayOffset) {
  const order = getGyobunOrder(team);
  const baseIdx = order.findIndex((code) => normalizeCodeKey(code) === normalizeCodeKey(baseCode));
...
function cloneTeamData(data) {
  ...
  TEAM_ORDER.forEach((teamKey) => {
    ...
    result[teamKey] = {
      ...
      people: Array.isArray(team.people) ? team.people.map((p) => ({
        ...p
      })) : [],
      info: team.info ? {
        ...team.info,
...
function parseZipToData(parsedFiles) {
  ...
  TEAM_ORDER.forEach((teamKey) => {
    ...
    const filtered = team.names.map((name, idx) => ({
      name,
...
function normalizeRemoteRosterShape(input) {
  ...
  if (TEAM_ORDER.some((teamKey) => Array.isArray(input?.[teamKey]))) {
    TEAM_ORDER.forEach((teamKey) => {
      result[teamKey] = (Array.isArray(input?.[teamKey]) ? input[teamKey] : []).map((row) => ({
...
function buildAssignedGrid(team, anchorName, anchorCode, dayOffset, overrides) {
  ...
  const anchorCodeIndex = fixedCodes.findIndex((code) => normalizeCodeKey(code) === normalizeCodeKey(anchorCode));
  if (anchorPersonIndex < 0 || anchorCodeIndex < 0) {
    return fixedCodes.map((slotCode, slotIndex) => {
      const person = people[slotIndex] || {
        idx: slotIndex,
        name: ""
      };
  ...
  return fixedCodes.map((slotCode, slotIndex) => {
    const personIndex = positiveMod(anchorPersonIndex + (slotIndex - anchorCodeIndex - dayOffset), people.length);
...
function buildTeamAnchorFromZip(team) {
  ...
  const fixedCodes = getGyobunOrder(team);
  let baseDate = getZipBaseDate(team);
  ...
  if (matchedPerson) return {
    ...
    code: normalizeToFixedCode(team, team?.info?.baseCode || matchedPerson.baseCode || fixedCodes[0] || ""),
    anchorDate: baseDate
  };
  const firstPerson = people[0];
  return {
    name: firstPerson?.name || "",
    code: normalizeToFixedCode(team, team?.info?.baseCode || firstPerson?.baseCode || fixedCodes[0] || ""),
    anchorDate: baseDate
  };
}
...
function applyRemoteRosterNamesForSetup(baseData, remoteRoster) {
  ...
  TEAM_ORDER.forEach((teamKey) => {
    ...
    const mapped = fixedOrder.map((slotCode, idx) => {
      ...
      const fallback = originalPeople.find((p) => normalizeCodeKey(p.baseCode) === normalizeCodeKey(slotCode)) || originalPeople[idx] || {
        idx,
        name: "",
        baseCode: slotCode,
        employeeId: ""
...
function buildAnchorForIdentity(teamKey, team, remoteRoster, name, mySelection = null) {
  ...
  if (mySelection?.teamKey === teamKey && samePersonName(mySelection?.name, name)) return {
    name,
    code: normalizeToFixedCode(team, mySelection?.code || ""),
    anchorDate: String(mySelection?.anchorDate || "").trim() || getZipBaseDate(team)
  };
...
function buildAllTeamsAutoAnchorsFromIdentity(data, remoteRoster, selectedTeamKey, selectedName, mySelection = null) {
  ...
  TEAM_ORDER.forEach((teamKey) => {
    ...
    if (teamKey === selectedTeamKey && selectedName) {
      result[teamKey] = buildAnchorForIdentity(teamKey, team, remoteRoster, selectedName, mySelection);
      return;
    }
...
function App() {
  ...
  const [mySelection, setMySelection] = useState(initialSelection || {
    teamKey: "ks",
    name: "",
    code: "",
    anchorDate: todayStr
  ...
  const groupAddCandidates = useMemo(() => {
    ...
    if (hasRemoteRosterForTeam(groupAddTeam, remoteRoster)) {
      baseList = remoteRoster[groupAddTeam].map(r => ({
        name: r.name
      }));
    } else {
      baseList = team.people || [];
    }
    return baseList.filter(p => p.name && !shouldHideName(p.name)).map(p => {
      const override = overrides[getOverrideKey(groupAddTeam, p.name)] || {};
      return {
        name: p.name,
        displayName: override.alias || p.name
      };
    });
  ...
  useEffect(() => {
    if (remoteBaseDate) {
      ...
      saveCachedSharedConfig({ ...prevConfig,
  ...
  }, [allowProfileEdit, draftTeam, draftName, draftCode, remoteRoster, setupSourceData, data, ]);
  useEffect(() => {
  ...
  function acceptRemoteRoster(json, options = {}) {
    const {
      alertMessage = "", nextDataOverride = null, syncMine = true
    } = options;
    ...
    let effectiveDate = String(json?.effectiveDate || json?.date || json?.rosterDate || json?.snapshotDate || json?.currentDate || "").trim();
    if (!effectiveDate) effectiveDate = getKoreaToday();
  ...
  useEffect(() => {
    ...
    async function initAppFast() {
      ...
      try {
        ...
        await Promise.all(preloadYears.map((year) => ensureHolidayYear(year, () => {
  ...
  useEffect(() => {
    ...
    async function checkRemoteAfterSetup() {
      ...
      try {
        ...
        const hasAny = hasAnyRemoteRoster(next);
        const nextSig = getRemoteRosterSignature(next);
        const currentAckSig = localStorage.getItem(LS_LAST_ACK_ROSTER_SIG) || "";
  ...
  useEffect(() => {
    showGroupAddRef.current = showGroupAdd;
  }, [showGroupAdd]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!window.history.state || !window.history.state.__gyobeon) window.history.replaceState({
      __gyobeon: true,
      layer: "root"
    }, "");
  ...
  useEffect(() => {
    if (pathOpen && (!window.history.state || window.history.state.layer !== "path")) window.history.pushState({
      __gyobeon: true,
      layer: "path"
    }, "");
  }, [pathOpen]);
  useEffect(() => {
    if (editOpen && (!window.history.state || window.history.state.layer !== "edit")) window.history.pushState({
      __gyobeon: true,
      layer: "edit"
    }, "");
  }, [editOpen]);
  useEffect(() => {
    if (showUpdatePopup && (!window.history.state || window.history.state.layer !== "update")) window.history.pushState({
      __gyobeon: true,
      layer: "update"
    }, "");
  }, [showUpdatePopup]);
  useEffect(() => {
    if (showGroupAdd && (!window.history.state || window.history.state.layer !== "groupAdd")) window.history.pushState({
      __gyobeon: true,
      layer: "groupAdd"
    }, "");
  }, [showGroupAdd]);
  useEffect(() => {
    if (showSettings && (!window.history.state || window.history.state.layer !== "settings")) window.history.pushState({
      __gyobeon: true,
      layer: "settings"
    }, "");
  }, [showSettings]);
  useEffect(() => {
    ...
    if (mySelection?.teamKey && String(mySelection?.name || "").trim()) {
      const autoAnchors = buildAllTeamsAutoAnchorsFromIdentity(effectiveData, remoteRoster, mySelection.teamKey, mySelection.name, mySelection);
      setTeamAnchors(autoAnchors);
      setSelectedTeam(mySelection.teamKey);
      if (activeTabRef.current === "home") setViewTeam(mySelection.teamKey);
      return;
    }
    const nextAnchors = {};
    TEAM_ORDER.forEach((teamKey) => {
  ...
  const myInfo = useMemo(() => {
    ...
    const anchor = buildAnchorForIdentity(myTeamKey, team, remoteRoster, myName, mySelection);
    if (!anchor?.code) return null;
    const dayOffset = diffDays(anchor.anchorDate || getResolvedBaseDate(myTeamKey, team, remoteRoster), homeDate);
  ...
  const allGrid = useMemo(() => {
    ...
    if (mySelection?.teamKey === viewTeam && mySelection?.code && String(mySelection?.name || "").trim() && !hasRemoteRosterForTeam(viewTeam, remoteRoster)) {
      ...
      grid = grid.map((cell) => {
        if (normalizeToFixedCode(currentViewTeam, cell.code) === myCode) return { ...cell,
  ...
  /** 🚀 검색 로직 최종 보정: 2000번대 열번 검색 시 searchPathDate를 명시하여 이미지 호출 시 '어제' 날짜 사용 보장 **/
  const filteredGrid = useMemo(() => {
    ...
    TEAM_ORDER.forEach(teamKey => {
      const team = effectiveData[teamKey];
      if (!team) return;
      const teamGrid = hasRemoteRosterForTeam(teamKey, remoteRoster) ? buildRemoteShiftedGrid(teamKey, team, remoteRoster, browseDate, overrides) : buildAssignedGrid(team, teamAnchors[teamKey]?.name, teamAnchors[teamKey]?.code, diffDays(teamAnchors[teamKey]?.anchorDate, browseDate), overrides);
      const yesterdayGrid = hasRemoteRosterForTeam(teamKey, remoteRoster) ? buildRemoteShiftedGrid(teamKey, team, remoteRoster, yesterdayStr, overrides) : buildAssignedGrid(team, teamAnchors[teamKey]?.name, teamAnchors[teamKey]?.code, diffDays(teamAnchors[teamKey]?.anchorDate, yesterdayStr), overrides);
  ...
  const diaList = useMemo(() => {
    ...
    if (hasRemoteRosterForTeam(viewTeam, remoteRoster)) {
      grid = buildRemoteShiftedGrid(viewTeam, team, remoteRoster, browseDate, overrides);
    } else if (canUseMyAnchorForTeam) {
      grid = buildAssignedGrid(team, mySelection.name, normalizeToFixedCode(team, mySelection.code), diffDays(mySelection.anchorDate || getResolvedBaseDate(viewTeam, team, remoteRoster), browseDate), overrides);
    } else {
      const teamAnchor = buildTeamAnchorFromZip(currentViewTeam);
      grid = buildAssignedGrid(team, teamAnchor.name, teamAnchor.code, diffDays(teamAnchor.anchorDate || getResolvedBaseDate(viewTeam, team, remoteRoster), browseDate), overrides);
    }
    if (viewTeam === mySelection?.teamKey && mySelection?.code && String(mySelection?.name || "").trim() && !hasRemoteRosterForTeam(viewTeam, remoteRoster)) {
      const myCode = normalizeToFixedCode(currentViewTeam, getMyCodeForDate(currentViewTeam, browseDate, mySelection));
      grid = grid.map((cell) => {
        if (normalizeToFixedCode(currentViewTeam, cell.code) === myCode) return { ...cell,
          name: mySelection.name,
    ...
    const diaOrder = findDiaOrder(team);
    return diaOrder.map((code) => {
      const found = grid.find((item) => normalizeCodeKey(item.code) === normalizeCodeKey(code));
      return {
        code,
        idx: found?.idx ?? -1,
        name: found?.name || "-",
  ...
  useEffect(() => {
    ...
    if (!selectedGroupDate || !weekDates.includes(selectedGroupDate)) setSelectedGroupDate(weekDates[0]);
  ...
  function handleGroupMonthChange(nextMonthValue) {
    ...
    if (nextMonthValue === todayMonth) setGroupBaseDate(today);
    else setGroupBaseDate(getMonthStartDate(nextMonthValue));
  }
  ...
  function switchTab(tabName) {
    ...
    if (tabName === "home") window.history.pushState({
      __gyobeon: true,
      layer: "root"
    }, "");
  ...
  async function parseAndSetZip(fileOrBlob, saveToIdb = true, keepSavedSelection = false, rosterForApply = remoteRoster, showBusy = true) {
    ...
    try {
      ...
      zip.forEach((relativePath, entry) => {
        ...
        if (lower.endsWith(".txt")) tasks.push(entry.async("string").then((text) => {
          parsedFiles[relativePath] = text;
        }));
        else if (/\.(png|jpg|jpeg)$/i.test(lower)) tasks.push(entry.async("base64").then((base64) => {
      ...
      if (!keepSavedSelection) {
        ...
        const autoAnchors = buildAllTeamsAutoAnchorsFromIdentity(nextData, rosterForApply || getEmptyRemoteRoster(), defaultTeam, defaultName, mySelection);
        setTeamAnchors(autoAnchors);
  ...
  function handleAllCellTap(item) {
    if (editMode) openEditDialog(item);
      ...
      const targetDate = item.searchPathDate || browseDate;
  ...
  function openEditDialog(item) {
    setEditingCell(item);
    const currentTeam = item.teamKey || viewTeam;
    const key = getOverrideKey(currentTeam, item.name);
    const current = overrides[key] || {};
    setEditColor(current.color || "");
    setEditAlias(current.alias || "");
    const team = effectiveData?.[currentTeam];
  ...
  function closeEditDialog() {
    if (editOpenRef.current) window.history.back();
  ...
  function commitEdit(nextColorValue = editColor, nextAliasValue = editAlias) {
    ...
    if (!cleanColor && !cleanAlias) delete next[key];
    else next[key] = {
      color: cleanColor,
      alias: cleanAlias
    };
    if (isWorktimeEditOpen) {
      const built = buildTimeValueFromParts(editStartHour, editStartMin, editEndHour, editEndMin);
      if (!built) return alert("출퇴근시간 형식을 다시 확인해주세요.");
      const dayType = guessDayType(browseDate);
      const allWorktimeOverrides = loadWorktimeOverrides();
      const wtKey = getWorktimeOverrideKey(currentTeam, editingCell.code);
  ...
  function closePathDialog() {
    if (pathOpenRef.current) window.history.back();
  ...
  function addToGroup() {
    ...
    if (!exists) next[targetGroup].push({
      team: groupAddTeam,
      name: groupAddName
    });
    setGroups(next);
  ...
  function removeFromGroup(teamKey, name) {
    ...
    next[currentGroup] = (next[currentGroup] || []).filter((item) => !(item.team === teamKey && samePersonName(item.name, name)));
    setGroups(next);
    saveGroups(next);
  }
  ...
  function importSettings(e) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (showSettingsRef.current) window.history.back();
                        ...
                        return (
                          <div key={`${idx}-${item.code}-${item.displayName}-${item.teamKey}`} className={`all-cell-real ${isMine ? "cell-my" : ""} ${isMine && isToday ? "cell-my-today" : ""}`} style={customStyle} onClick={() => handleAllCellTap(item)}>
                            ...
                            <div className="all-name" style={textColorStyle}>
                                {item.displayName || "-"}
                                {searchQuery && <div style={{fontSize: '9px', opacity: 0.8, color: item.customColor ? '#000000' : 'inherit'}}>[{TEAM_LABELS[item.teamKey]}]</div>}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    {searchQuery && visibleAllGrid.length > 0 && (
                      <div className="search-img-panel" style={{ marginTop: '20px', paddingBottom: '30px' }}>
                        {visibleAllGrid.map((item, idx) => {
                          ...
                          const imgDate = item.searchPathDate || browseDate;
                          ...
                          return (
                            <div key={`s-img-${idx}`} style={{ marginBottom: '20px', textAlign: 'center' }}>
                              <div style={{ fontSize: '12px', opacity: 0.7, marginBottom: '8px' }}>🔍 {item.displayName} ({item.code}) 행로표</div>
            ...
            {activeTab === "month" && (
              <div className="tab-page" id="capture-month-area">
                ...
                <div className="month-calendar" style={swipeStyle}>
                  <div className="month-weekdays"><div className="sun">일</div><div>월</div><div>화</div><div>수</div><div>목</div><div>금</div><div className="sat">토</div></div>
              ...
              <div className="group-page tab-page">
                <div className="group-top-bar-v4">
                  ...
                  <div className="group-select-wrap">
                    ...
                    <select className="group-select-overlay" value={groupMonth} onChange={(e) => handleGroupMonthChange(e.target.value)}>{groupMonthOptions.map((item) => (<option key={item.value} value={item.value}>{item.label}</option>))}</select>
                  </div>
                  <div className="group-select-wrap">
                    <div className="group-select-display">{currentGroup ? `${currentGroup} ▾` : "그룹 없음 ▾"}</div>
                    <select className="group-select-overlay" value={currentGroup} onChange={(e) => setCurrentGroup(e.target.value)}>{Object.keys(groups).map((g) => <option key={g} value={g}>{g}</option>)}</select>
                  </div>
                    ...
                    <tbody>
                      {groupMembers.length === 0 ? (<tr><td colSpan={8} className="empty-msg">그룹 인원을 추가해주세요.</td></tr>) : (
                        groupMembers.map((member, idx) => {
                          ...
                          return (
                          <tr key={`${idx}`}>
                            <td className="group-name-cell sticky-col"><div className="group-name-cell-inner"><div className="name-txt">{displayMemberName}</div><div className="team-badge">{TEAM_LABELS[member.team]}</div><button className="row-del-btn-text" onClick={() => removeFromGroup(member.team, member.name)}>삭제</button></div></td>
                            {weekDates.map((date) => {
                              const item = getPersonGyobunForDate(effectiveData, remoteRoster, member.team, member.name, date, overrides, mySelection); const isSelectedCol = selectedGroupDate === date;
                              return (
                                <td key={date} onClick={() => { setSelectedGroupDate(date); if (item?.code) { openPathDialogForTeamAndDate(member.team, { code: item.code, name: member.name, displayName: displayMemberName, idx: -1 }, date); } }} style={{ cursor: "pointer", padding: 0, overflow: "hidden", background: isSelectedCol ? (isDarkMode ? "rgba(59, 130, 246, 0.2)" : "#f5f3ff") : "", transition: "background-color 0.18s ease" }}>
                                  <div style={{ ...swipeStyle, padding: '8px 4px', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', fontWeight: isSelectedCol ? 700 : 600, color: isSelectedCol ? (isDarkMode ? "#60a5fa" : "#4c1d95") : "inherit" }}>{item?.code || "-"}</div>
                                </td>
                              );
                            })}
                          </tr>
      ...
      {showSettings && (
        <div className="modal-backdrop" onClick={() => { if (showSettingsRef.current) window.history.back(); else setShowSettings(false); }}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label className="label">화면 테마</label>
            <button className="modal-btn" style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', marginBottom: 16 }} onClick={() => setIsDarkMode(!isDarkMode)}>
              <span>{isDarkMode ? "🌙 다크 모드 켜짐" : "☀️ 라이트 모드 켜짐"}</span><span style={{ fontSize: '18px' }}>{isDarkMode ? "✅" : "☑️"}</span>
            </button>
            ...
            {!allowProfileEdit ? (
              <>
                ...
                <div className="notice-box" style={{ marginTop: 8 }}>내 소속: {TEAM_LABELS[mySelection?.teamKey || selectedTeam] || "-"}<br />내 이름: {mySelection?.name || "-"}<br />내 기준교번: {mySelection?.code || "-"}<br />기준날짜: {mySelection?.anchorDate || "-"}</div>
              ...
              <>
                ...
                <select className="select" value={draftTeam} onChange={(e) => { const nextTeam = e.target.value; setDraftTeam(nextTeam); setSelectedTeam(nextTeam); setDraftName(""); setDraftCode(""); }}>{TEAM_ORDER.map((key) => (<option key={key} value={key}>{TEAM_LABELS[key]}</option>))}</select>
                <label className="label" style={{ marginTop: 12 }}>내 이름</label>
                <input className="input" type="text" placeholder="이름 직접 입력" value={draftName} onChange={(e) => { setDraftName(e.target.value); setDraftCode(""); }} />
                <label className="label" style={{ marginTop: 12 }}>오늘 교번</label>
                <select className="select" value={draftCode} onChange={(e) => { setDraftCode(e.target.value); }}><option value="">선택</option>{(setupSourceData?.[draftTeam]?.gyobun || DEFAULT_GYOBUN).map((code, idx) => (<option key={`${idx}`} value={code}>{code}</option>))}</select>
                <label className="label" style={{ marginTop: 12 }}>기준 날짜</label>
                <input className="input" type="date" value={profileAnchorDate} onChange={(e) => { const nextDate = e.target.value || getKoreaToday(); setProfileAnchorDate(nextDate); }} />
                <div className="modal-actions"><button className="modal-btn" onClick={cancelReconfigureProfile}>취소</button><button className="modal-btn primary" onClick={() => applyInitialSelection(draftTeam, draftName, draftCode)} disabled={!String(draftName || "").trim() || !draftCode}>저장</button></div>
              </>
            )}
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              ...
              <label className="modal-btn" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', margin: 0 }}>📤 복구하기<input type="file" accept=".json" style={{ display: 'none' }} onChange={importSettings} /></label>
            </div>
            {isAdminUser && (
              <div className="card" style={{ marginTop: 14, padding: 12 }}>
                ...
                <div className="modal-actions"><button className="modal-btn" onClick={publishRoster} disabled={savingSharedConfig}>{savingSharedConfig ? "처리중..." : "현재배정 배포"}</button><button className="modal-btn primary" onClick={saveSharedConfig} disabled={savingSharedConfig}>{savingSharedConfig ? "저장중..." : "공용 기준일 저장"}</button></div>
              </div>
            )}
            <div className="modal-actions"><button className="modal-btn" onClick={resetMyProfile}>내 정보 초기화</button><button className="modal-btn primary" onClick={() => { if (showSettingsRef.current) window.history.back(); else setShowSettings(false); }}>닫기</button></div>
          </div>
        </div>
      )}
      {showGroupAdd && (
        <div className="modal-backdrop" onClick={() => { if (showGroupAddRef.current) window.history.back(); else setShowGroupAdd(false); }}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}><input className="input" style={{ flex: 1 }} value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} placeholder="예: 1조" /><button className="modal-btn primary" style={{ width: 'auto', padding: '0 16px' }} onClick={handleGroupSubmit}>생성</button></div>
            <label className="label">2. 관리할 그룹 선택</label>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '20px' }}>
              <select className="select" style={{ flex: 1, margin: 0 }} value={currentGroup} onChange={(e) => setCurrentGroup(e.target.value)}>{Object.keys(groups).length === 0 ? (<option value="">그룹 없음</option>) : (Object.keys(groups).map((g) => <option key={g} value={g}>{g}</option>))}</select>
            ...
            <label className="label">3. 인원 추가</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
              <select className="select" value={groupAddTeam} onChange={(e) => { setGroupAddTeam(e.target.value); setGroupAddName(""); }}>{TEAM_ORDER.map((key) => (<option key={key} value={key}>{TEAM_LABELS[key]}</option>))}</select>
              <select className="select" value={groupAddName} onChange={(e) => setGroupAddName(e.target.value)}><option value="">선택</option>{groupAddCandidates.map((person) => (<option key={`${groupAddTeam}-${person.name}`} value={person.name}>{person.displayName}</option>))}</select>
            </div>
            <button className="modal-btn primary" style={{ width: '100%' }} onClick={addToGroup} disabled={!currentGroup || !groupAddName}>+ 인원 추가</button>
            <div className="modal-actions" style={{ marginTop: '24px' }}><button className="modal-btn" style={{ width: '100%' }} onClick={() => { if (showGroupAddRef.current) window.history.back(); else setShowGroupAdd(false); }}>닫기</button></div>
          </div>
        </div>
      )}
      {editOpen && (
        <div className="modal-backdrop" onClick={closeEditDialog}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
                {COLOR_OPTIONS.map((item) => (<button key={item.label} className={`color-dot ${editColor === item.value ? 'active' : ''}`} style={{ backgroundColor: item.value || '#ffffff', border: item.value ? 'none' : '1px solid #ddd', width: 40, height: 40, borderRadius: '50%' }} onClick={() => setEditColor(item.value)} title={item.label} />))}
            </div>
            ...
            {isWorktimeEditOpen && (
              <div style={{ marginTop: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}><input className="input" inputMode="numeric" value={editStartHour} onChange={(e) => setEditStartHour(clamp2(e.target.value))} style={{ textAlign: "center" }} placeholder="06" />: <input className="input" inputMode="numeric" value={editStartMin} onChange={(e) => setEditStartMin(clamp2(e.target.value))} style={{ textAlign: "center" }} placeholder="33" /></div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}><input className="input" inputMode="numeric" value={editEndHour} onChange={(e) => setEditEndHour(clamp2(e.target.value))} style={{ textAlign: "center" }} placeholder="15" />: <input className="input" inputMode="numeric" value={editEndMin} onChange={(e) => setEditEndMin(clamp2(e.target.value))} style={{ textAlign: "center" }} placeholder="54" /></div>
              </div>
            )}
            <div className="modal-actions"><button className="modal-btn" onClick={closeEditDialog}>아니요</button><button className="modal-btn primary" onClick={() => commitEdit(editColor, editAlias)}>변경</button></div>
          </div>
        </div>
      )}
      {pathOpen && (
      ...
      {showUpdatePopup && (
        <div className="modal-backdrop" onClick={closeUpdatePopup}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions"><button className="modal-btn" onClick={closeUpdatePopup}>나중에</button><button className="modal-btn primary" onClick={applyPendingRosterUpdate}>업데이트</button></div>
          </div>
        </div>
      )}
    </>
